...
use crate::util::{Answer, Event, NewEvent, Ordinal, Tag};
...
impl SqliteDb {
    ...
    pub fn read_from(&self, ordinal: Ordinal) -> Result<Vec<Event>, rusqlite::Error> {
        let mut stmt = self.con.prepare("SELECT * FROM events WHERE ordinal >= ?1")?;
        let events = stmt.query_and_then([&ordinal.0], row_to_event)?;
...
fn row_to_event(row: &Row) -> Result<Event, rusqlite::Error>{
    ...
    let answer: Option<Vec<u8>> = row.get(5)?; // вот только answer - это структура а не байты
    let answer = answer.map(Answer::try_from);
    if let Some(Ok(answer)) = answer {
        Ok(Event::from_values(Ordinal(ordinal), Tag(tag), external, name, data, Some(answer)))
    } else if answer.is_none() {
        Ok(Event::from_values(Ordinal(ordinal), Tag(tag), external, name, data, None))
    } else {
